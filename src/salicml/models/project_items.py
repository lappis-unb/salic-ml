...
class ProjectItems:
    ...
    def items(self, id_pronac, dt=None):
        data = self.get_rows_by_column_value(ProjectItems.ID_PRONAC, id_pronac, dt=dt)
        return data[ProjectItems.ID_ITEM].unique()

    def areas_id(self, id_pronac):
        data = self.get_rows_by_column_value(ProjectItems.ID_PRONAC, id_pronac)
    ...
    def projects_similarity(self, itens_a, itens_b):
        ...
        intersction_size = np.intersect1d(itens_a, itens_b, assume_unique=True).size
...
class Projects:
    ...
    def most_frequent_area_segment(self):
        assert self.dt is not None
        res = (
            self.dt.groupby([Projects.AREA, Projects.SEGMENTO])
            .size()
