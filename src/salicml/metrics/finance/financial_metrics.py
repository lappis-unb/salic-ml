...
class FinancialMetrics:
    PROCESSED_FILE_PATH = os.path.join(
        PROJECT_ROOT, "data", "processed", "financial_metrics.pickle"
    )
    ...
    def get_metrics(self, pronac, metrics=None):
        ...
        for metric in metrics:
            try:
                ...
                print(
                    "No data for pronac={} for metric={}, skipping it.".format(
                        pronac, metric
                    )
                )
    ...
    def calculate_easiness(self, metrics):
        ...
        for metric_key, metric in metrics.items():
            if IS_OUTLIER_KEY in metric:
                total_metrics += 1
                total_metrics_outliers += 1 if metric[IS_OUTLIER_KEY] else 0
    ...
    def _init_datasets(self):
        ...
        self.datasets = {
            "orcamento": datasource.get_dataset(
                os.path.join(sql_folder, dataset_sql_map["orcamento"]), use_cache=True
            ),
            "comprovacao": datasource.get_dataset(
                os.path.join(sql_folder, dataset_sql_map["comprovacao"]), use_cache=True
            ),
            "captacao": datasource.get_dataset(
                os.path.join(sql_folder, dataset_sql_map["captacao"]), use_cache=True
            ),
            "projetos": datasource.get_dataset(
                os.path.join(sql_folder, dataset_sql_map["projetos"]), use_cache=True
            ),
        }
    ...
    def _init_metrics(self):
        self.metrics = {
            ...
            "common_items_ratio": CommonItemsRatio(
                self.datasets["orcamento"].copy(), self.datasets["comprovacao"].copy()
            ),
            "total_receipts": TotalReceipts(self.datasets["comprovacao"].copy()),
            "new_providers": NewProviders(self.datasets["comprovacao"].copy()),
            "proponent_projects": ProponentProjects(
                self.datasets["comprovacao"].copy(), self.datasets["projetos"].copy()
            ),
            "items_prices": ItemsPrice(
                self.datasets["orcamento"].copy(), self.datasets["comprovacao"].copy()
            ),
        }
