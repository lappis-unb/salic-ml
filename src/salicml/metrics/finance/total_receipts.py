...
class TotalReceipts:
    ...
    def get_metrics(self, pronac):
        ...
        is_outlier, total_receipts, maximum_expected = self.is_pronac_outlier(pronac)
    ...
    def _init_metrics_cache(self):
        segment_projects = self.dt_total_receipts.groupby(
            ["idSegmento", "PRONAC"]
        ).nunique()

        segment_projects_std = (
            segment_projects["idComprovantePagamento"]
            .groupby(["idSegmento"])
    ...
    def _init_projects_data(self):
        ...
        self.project_receipts.drop(columns=["PRONAC", "idSegmento"], inplace=True)

        self.project_receipts.rename(
            columns={"idComprovantePagamento": "NumeroComprovantes"}, inplace=True
        )
