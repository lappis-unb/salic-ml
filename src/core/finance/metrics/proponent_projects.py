...
class ProponentProjects:
    ...
    def get_metrics(self, pronac):
        ...
        if cnpj_cpf in self.submitted_projects.keys():
            submitted["number_of_projects"] = self.submitted_projects[cnpj_cpf][
                "num_pronacs"
            ]
            submitted["pronacs_of_this_proponent"] = self.submitted_projects[cnpj_cpf][
                "pronacs_list"
            ]
        ...
        if cnpj_cpf in self.analyzed_projects.keys():
            analyzed["number_of_projects"] = self.analyzed_projects[cnpj_cpf][
                "num_pronacs"
            ]
            analyzed["pronacs_of_this_proponent"] = self.analyzed_projects[cnpj_cpf][
                "pronacs_list"
            ].tolist()
    ...
    def get_cnpj_cpf_by_pronac(self, pronac):
        ...
        cnpj_cpf_df = self.df_projects.loc[
            self.df_projects["PRONAC"] == pronac, ["CgcCpf"]
        ]
    ...
    def _set_up_cache(self):
        ...
        analyzed_projects = filtered_verified_funds.groupby(["proponenteCgcCpf"])[
            "PRONAC"
        ].agg(["unique", "nunique"])
        ...
        submitted_projects = filtered_projects.groupby(["CgcCpf"])["PRONAC"].agg(
            ["unique", "nunique"]
        )
    ...
    def _get_pronac_data(self, pronac):
        ...
        cnpj_cpf = "CNPJ/CPF NOT FOUND"
        cnpj_cpf_df = pronac_dataframe.loc[
            pronac_dataframe["PRONAC"] == pronac, ["CgcCpf"]
        ]
        if not cnpj_cpf_df.empty:
            cnpj_cpf = cnpj_cpf_df.iloc[0, 0]
