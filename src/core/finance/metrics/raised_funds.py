...
class RaisedFunds:
    def __init__(self, dt_raised_funds):
        ...
        dt_raised_funds["CaptacaoReal"] = dt_raised_funds["CaptacaoReal"].apply(
            pd.to_numeric
        )
    ...
    def _init_metrics_cache(self):
        """ TODO
        """
        pronac_segment_projects = (
            self.dt_raised_funds[["Pronac", "Segmento", "CaptacaoReal"]]
    ...
    def _get_pronac_data(self, pronac):
        ...
        pronac_dataframe["CaptacaoReal"] = pronac_dataframe["CaptacaoReal"].apply(
            pd.to_numeric
        )

        pronac_data = {
            "segment_id": pronac_dataframe.iloc[0]["Segmento"],
            "raised_funds": pronac_dataframe[["Pronac", "CaptacaoReal"]]
            .groupby(["Pronac"])
            .sum()
            .loc[pronac]["CaptacaoReal"],
        }
