...
class VerifiedFunds:
    ...
    def __init__(self, dt_verified_funds):
        ...
        dt_verified_funds["vlComprovacao"] = dt_verified_funds["vlComprovacao"].apply(
            pd.to_numeric
        )
    ...
    def _init_metrics_cache(self):
        ...
        segment_projects = (
            self.dt_verified_funds[["PRONAC", "idSegmento", "vlComprovacao"]]
    ...
    def _init_projects_data(self):
        ...
        self._dt_projects = self.dt_verified_funds[
            ["idPlanilhaAprovacao", "PRONAC", "vlComprovacao", "idSegmento"]
        ]

        self.project_funds_grp = self._dt_projects.drop(
            columns=["idPlanilhaAprovacao"]
        ).groupby(["PRONAC"])
    ...
    def is_pronac_outlier(self, pronac):
        ...
        if not pronac_data["segment_id"] in self._segments_cache:
            raise ValueError(
                "Segment {} was not trained".format(pronac_data["segment_id"])
            )
    ...
    def _get_pronac_data(self, pronac):
        ...
        pronac_dataframe["vlComprovacao"] = pronac_dataframe["vlComprovacao"].apply(
            pd.to_numeric
        )

        pronac_funds = pronac_dataframe[
            ["idPlanilhaAprovacao", "PRONAC", "vlComprovacao", "idSegmento"]
        ]
