...
class TestProponentProjects(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dt_verified_funds = read_csv_with_different_type(
            csv_name, {"PRONAC": str, "proponenteCgcCpf": str}, usecols=usecols
        )

        csv_name = "planilha_projetos.csv"
        usecols = ["PRONAC", "CgcCpf"]
        cls.dt_projects = read_csv_with_different_type(
            csv_name, {"PRONAC": str, "CgcCpf": str}, usecols=usecols
        )

        cls.proponent_projects = ProponentProjects(
            cls.dt_verified_funds, cls.dt_projects
        )

    def test_IO(self):
        csv_name = "planilha_comprovacao.csv"
        usecols = ["PRONAC", "proponenteCgcCpf"]
        csv = read_csv_with_different_type(
            csv_name, {"PRONAC": str, "proponenteCgcCpf": str}, usecols=usecols
        )
